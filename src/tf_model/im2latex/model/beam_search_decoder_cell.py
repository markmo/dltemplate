...
from tf_model.im2latex.model.greedy_decoder_cell import DecoderOutput


class BeamSearchDecoderCellState(
        collections.namedtuple('BeamSearchDecoderCellState', ('cell_state', 'log_probs'))):
    """
    State of the beam search decoding

    cell_state: cell state for all hypotheses, shape [batch_size, beam_size, ?]
    embedding: embeddings of the previous time step for each hypothesis,
               shape [batch_size, beam_size, embedding_size]
    log_probs: log probabilities of hypotheses, shape [batch_size, beam_size]
    finished: (bool) has a beam hypothesis reached token id_end
    """
    pass


class BeamSearchDecoderOutput(
        collections.namedtuple('BeamSearchDecoderOutput', ('logits', 'ids', 'parents'))):
    """
    Stores the logic for the beam search decoding

    logits: scores before softmax of the beam search hypotheses,
            shape [batch_size, beam_size, vocab_size]
    ids: ids of the best words at this time step, shape [batch_size, beam_size]
    parents: ids of the beam index from previous time step, shape [batch_size, beam_size]
    """
    pass
...
class BeamSearchDecoderCell(object):
    ...
    def __init__(self, embeddings, cell, batch_size, start_token, end_token,
        ...
        :param embeddings: (tf.Variable) shape (vocab_size, embedding_size)
        :param cell: instance of Cell that defines a step function
        :param batch_size: (tf.int) extracted with tf.Shape or int
        :param start_token: (int) id of start token
        :param end_token: (int) id of the end token
        :param beam_size: (int)
        :param div_gamma: (float) amount of penalty to add to beam hypothesis
                          for diversity. Coefficient of penalty will be
                          log(div_gamma). Use value between 0 and 1. (1 means
                          no penalty.)
        :param div_prob: apply div penalty with probability div_prob.
                         (0 means never apply penalty.)
        """
    ...
    def output_dtype(self):
        ...
        :return:
        """
        return BeamSearchDecoderOutput(logits=self._cell.output_dtype, ids=tf.int32, parents=tf.int32)
    ...
    @property
    def state_size(self):
        return BeamSearchDecoderOutput(logits=tf.TensorShape([self._beam_size, self._vocab_size]),
                                       ids=tf.TensorShape([self._beam_size]),
                                       parents=tf.TensorShape([self._beam_size]))

    @property
    def final_output_size(self):
        return DecoderOutput(logits=tf.TensorShape([self._beam_size, self._vocab_size]),
                             ids=tf.TensorShape([self._beam_size]))
    ...
    def initial_state(self):
        ...
        cell_state = nest.map_structure(lambda t: tile_beam(t, self._beam_size), cell_state)

        # prepare other initial states
        log_probs = tf.zeros([self._batch_size, self._beam_size], dtype=self._cell.output_dtype)
    ...
    def step(self, time, state, embedding, finished):
        ...
        :param time: tensor or int
        :param state: shape [batch_size, beam_size, ...]
        :param embedding: shape [batch_size, beam_size, d]
        :param finished: shape [batch_size, beam_size, ...]
        :return:
        """
        # merge batch and beam dimension before calling step of cell
        ...
        new_cell_state = nest.map_structure(lambda t: split_batch_beam(t, self._beam_size), new_cell_state)
        ...
        log_probs_flat = tf.cond(time > 0, lambda: log_probs_flat, lambda: log_probs[:, 0])
        ...
        new_cell_state = nest.map_structure(lambda t: gather_helper(t, new_parents, self._batch_size, self._beam_size),
                                            new_cell_state)

        # create new state of decoder
        new_state = BeamSearchDecoderCellState(cell_state=new_cell_state, log_probs=new_probs)

        new_output = BeamSearchDecoderOutput(logits=new_logits, ids=new_ids, parents=new_parents)
    ...
    def finalize(self, final_outputs, final_state):
        ...
        :param final_outputs: shape [time dimension, batch_size, beam_size, d]
        :param final_state: (BeamSearchDecoderOutput)
        :return: shape [time, batch, beam, ...]
        """
        # reverse the time dimension
        max_iterations = tf.shape(final_outputs.ids)[0]
        final_outputs = nest.map_structure(lambda t: tf.reverse(t, axis=[0]), final_outputs)

        # initial states
        def create_ta(d):
            return tf.TensorArray(dtype=d, size=max_iterations)

        initial_time = tf.constant(0, dtype=tf.int32)
        initial_outputs_ta = nest.map_structure(create_ta, self.final_output_dtype)
        initial_parents = tf.tile(tf.expand_dims(tf.range(self._beam_size), axis=0),
                                  multiples=[self._batch_size, 1])

        # noinspection PyUnusedLocal
        def condition(time, outputs_ta, parents):
            return tf.less(time, max_iterations)
        ...
        def body(time, outputs_ta, parents):
            ...
            new_state = nest.map_structure(lambda t: gather_helper(t, parents, self._batch_size, self._beam_size),
                                           input_t)
...
def sample_bernoulli(p, s):
    """
    Samples a boolean tensor with shape = s according to a Bernoulli distribution
...
def add_div_penalty(log_probs, div_gamma, div_prob, batch_size, beam_size, vocab_size):
    """
    Adds penalty to beam hypothesis following this paper by Li et al. 2016,
    "A Simple, Fast Diverse Decoding Algorithm for Neural Generation"

    :param log_probs: (tensor of floats), shape [batch_size, beam_size, vocab_size]
    :param div_gamma: (float) diversity parameter
    :param div_prob: (float) adds penalty with probability div_prob
    :param batch_size:
    :param beam_size:
    :param vocab_size:
    :return:
...
def merge_batch_beam(t):
    ...
    :param t: shape [batch_size, beam_size, ...] whose dimensions after
              beam_size must be statically known
    :return: shape [batch_size * beam_size, ...]
    """
...
def split_batch_beam(t, beam_size):
    ...
    :param t: shape [batch_size * beam_size, ...]
    :param beam_size:
    :return: shape [batch_size, beam_size, ...]
    """
    if t.shape.ndims == 1:
...
def mask_probs(probs, end_token, finished):
    ...
    :param probs: shape [batch_size, beam_size, vocab_size]
    :param end_token: (int)
    :param finished: (tf.bool), shape [batch_size, beam_size]
    :return:
    """
    # one hot of shape [vocab_size]
    vocab_size = probs.shape[-1].value
    one_hot = tf.one_hot(end_token, vocab_size, on_value=0, off_value=probs.dtype.min, dtype=probs.dtype)
...
def gather_helper(t, indices, batch_size, beam_size):
    ...
    :param t: shape [batch_size, beam_size, d]
    :param indices: shape [batch_size, beam_size]
    :param batch_size:
    :param beam_size:
    :return: new_t: shape as t, except new_t[:, i] = t[:, new_parents[:, i]]
    """
