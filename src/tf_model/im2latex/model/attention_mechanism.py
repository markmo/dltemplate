...
class AttentionMechanism(object):
    """
    Compute attention over an image
    """

    def __init__(self, img, dim_e, tiles=1):
        """

        :param img: (tf.Tensor) image
        :param dim_e: (int) dimension of the intermediary vector
                      used to compute attention
        :param tiles: (int) default 1, input to context h, may
                      have size (tile * batch_size, ...)
        """
        if len(img.shape) == 3:
            self._img = img
        elif len(img.shape) == 4:
            m, h, w, _ = tf.shape(img)
            c = img.shape[3].value
            self._img = tf.reshape(img, shape=[m, h * w, c])
        else:
    ...
    def context(self, h):
        ...
        :param h: (batch_size, n_hidden_units) hidden state
        :return: c (batch_size, channels) context vector
        """
        with tf.variable_scope(self._scope_name):
            ...
            # works faster with a `matmul` than with a `*` and `tf.reduce_sum`
    ...
    def initial_cell_state(self, cell):
        ...
        :param cell: (instance of RNNCell) must define `state_size`
        :return:
        """
        _states_0 = []
        # noinspection PyProtectedMember
        for hidden_name in cell.state_size._fields:
            hidden_dim = getattr(cell.state_size, hidden_name)
