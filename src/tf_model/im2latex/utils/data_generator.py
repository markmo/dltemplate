...
class DataGenerator(object):
    """
    Generator of tuple (image, formula)
    """
    ...
    def __init__(self, path_formulas, dir_images, path_matching, bucket=False,
        ...
        :param path_formulas: (string) file of formulas
        :param dir_images: (string) dir of images, contains jpeg files
        :param path_matching: (string) file of name_of_img, id_formula
        :param bucket: (bool) flag if true, buckets the data by size
                       of image
        :param form_prepro: (lambda function) array -> array of int32.
                            Default: identity
        :param iter_mode: (string) the type returned by the iterator
                          ['data', 'full']
        :param img_prepro: (lambda function) array -> array.
                           Default: identity
        :param max_iter: (int) maximum numbers of elements in the dataset
        :param max_len: (int) maximum length of a formula in the dataset
                        if longer, then not yielded
    ...
    def _set_data_generator(self):
        """
        Sets iterable or generator of tuples (img_path, formula id)
    ...
    def bucket(self, bucket_size):
        ...
        :param bucket_size: (int)
        :return: bucketed_dataset [(img_path1, id1), ...]
        """
        ...
        for idx, (img, formula, img_path, formula_id) in enumerate(self):
            ...
            # if bucket is full, write it then empty it
    ...
    def _load_formulas(self, filename):
        """
        Loads text file with formulas in a dict

        :param filename: (string) path of formulas
        :return: (dict) dict[idx] = one formula
        """
    ...
    def _process_instance(self, example):
        ...
        :param example: tuple (img_path, formula_ids)
                        img_path: (string) path to image
                        formula_id: (int) formula id
        :return: img: depending on _img_prepro
        ...
        skip = self._max_len is not None and len(formula) > self._max_len

        return inst, skip

    def __iter__(self):
        """
        Yields tuple (img, formula)
    ...
    def build(self, quality=100, density=200, down_ratio=2, buckets=None, n_threads=4):
        ...
        :param quality: param for magick
        :param density: param for magick
        :param down_ratio: (int) downsampling ratio
        :param buckets: list of tuples (list of sizes) to produce similar
                        shape images
        :param n_threads:
        :return:
        """
