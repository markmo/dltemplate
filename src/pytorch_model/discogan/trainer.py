...
import os
from pytorch_model.discogan.model_setup import DiscriminatorCNN, DiscriminatorFC, GeneratorCNN, GeneratorFC
import torch
...
class Trainer(object):
    ...
    def build_model(self):
        if self.dataset == 'toy':
            ...
            self.G_AB = GeneratorCNN(a_channel, b_channel, conv_dims, deconv_dims, self.num_gpu)
            self.G_BA = GeneratorCNN(b_channel, a_channel, conv_dims, deconv_dims, self.num_gpu)
    ...
    def load_model(self):
        ...
        indices = [int(os.path.basename(path.split('.')[0].split('_')[-1])) for path in paths]
        self.start_step = max(indices)
    ...
    def train(self):
        ...
        optimizer_d = optimizer(chain(self.D_A.parameters(), self.D_B.parameters()),
                                lr=self.lr, betas=(self.beta1, self.beta2), weight_decay=self.weight_decay)
        optimizer_g = optimizer(chain(self.G_AB.parameters(), self.G_BA.parameters()),
        ...
        for step in trange(self.start_step, self.max_step):
            ...
            if x_A.size(0) != x_B.size(0):
                print('[!] Sampled datasets from A and B have different sizes. Try resampling...')
                continue
            ...
            if self.loss == 'log_prob':
                l_d_A_real = bce(self.D_A(x_A).view(-1), real_tensor)
                l_d_A_fake = bce(self.D_A(x_BA).view(-1), fake_tensor)
                l_d_B_real = bce(self.D_A(x_B).view(-1), real_tensor)
                l_d_B_fake = bce(self.D_B(x_AB).view(-1), fake_tensor)
            elif self.loss == 'least_square':
                l_d_A_real = 0.5 * torch.mean((self.D_A(x_A) - 1)**2)
                l_d_A_fake = 0.5 * torch.mean((self.D_A(x_BA))**2)

                l_d_B_real = 0.5 * torch.mean((self.D_B(x_B) - 1)**2)
                l_d_B_fake = 0.5 * torch.mean((self.D_B(x_AB))**2)
            else:
            ...
            if self.loss == 'log_prob':
                l_gan_A = bce(self.D_A(x_BA).view(-1), real_tensor)
                l_gan_B = bce(self.D_B(x_AB).view(-1), real_tensor)
            elif self.loss == 'least_square':
