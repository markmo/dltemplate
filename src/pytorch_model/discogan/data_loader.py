...
PIX2PIX_DATASETS = ['facades', 'cityscapes', 'maps', 'edges2shoes', 'edges2handbags']
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root, scale_size, data_type, skip_pix2pix_processing=False):
        ...
        if self.name in PIX2PIX_DATASETS and not skip_pix2pix_processing:
            pix2pix_split_images(root)

        self.paths = glob(os.path.join(root, '{}/*'.format(data_type)))
        if len(self.paths) == 0:
            raise Exception('No images found in {}'.format(root))
...
def get_loader(root, batch_size, scale_size, num_workers=2, skip_pix2pix_processing=False, shuffle=True):
    a_data_set = Dataset(root, scale_size, 'A', skip_pix2pix_processing)
    b_data_set = Dataset(root, scale_size, 'B', skip_pix2pix_processing)
